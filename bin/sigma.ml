...
module Sigma = struct
  ...
  let rec lookup_ctor (c:id) (s:t) : (typ * id) option =
    match s with
    | [] -> None
    | hd :: tl -> if String.equal hd.ctor_name c then Some (hd.ctor_type, hd.datatype_name)
  ...
  let restrict (d:id) (s:t) : t =
    List.filter ~f:(fun e -> cheat_compare e.datatype_name d = 0) s
  ...
  let ctors (dt:id) (s:t) : (id * (typ * id)) list =
    List.filter ~f:(fun e -> String.(=) dt e.datatype_name) s |>
