...
module Typecheck : Typecheck_Sig = struct
    ...
    let rec check_exp (s:Sigma.t) (g:Gamma.t) (e:exp) : typ =
      ...
      | EApp (e1, e2) ->
           ...
           | TArr (t1, t2) ->
               if not (equal_typ t_arg t1) then
                   type_error (sprintf
       ...
       | ELet (f, is_rec, xs, t_expected, e1, e2) ->
           ...
           (* Function binding.                                                           *)
              ...
              if not (equal_typ t_expected t_actual) then
                type_error
       ...
       | EMatch (e, bs) -> begin match check_exp s g e with
           | TBase d ->
               ...
               if (Util.all_eq (equal_typ) ts) && (List.length ts > 0) then List.hd_exn ts
       ...
       | EPFun ios ->
           ...
           else if not (Util.all_eq equal_typ ts) then
             type_error
     ...
     and check_evidence (s:Sigma.t) (g:Gamma.t) (t:typ) (es:exp list) : unit =
       List.iter ~f:(fun e ->
         if not (equal_typ t (check_exp s g e))
    ...
    let check_decl (s:Sigma.t) (g:Gamma.t) (d:decl) : Sigma.t * Gamma.t =
      ...
      | DLet (f, is_rec, xs, t_expected, e) ->
          ...
          if List.length xs = 0 then
              let t_actual = check_exp s g e in
              if equal_typ t_actual t_expected then
