...
module GTS : sig
  ...
  module T = struct
    ...
    let compare = cheat_compare   (* NOTE: use the built-in compare function *)
...
and gen_eexp_rel (tmo:Timeout.t) (s:Sigma.t)
  ...
  fetch_or_calculate memo_eexp_rel_tbl
    ...
    begin fun _ ->
      if met.size = 1 && equal_typ trel t then
...
and gen_eexp_rel_proj (tmo:Timeout.t) (s:Sigma.t)
  ...
  let rec contains_goal (t:typ) =
    if equal_typ t goal then true else
    begin match t with
  ...
  let rec path_to_goal_type (t:typ) (path:proj list) : proj list list =
    if equal_typ t goal then [path] else
    begin match t with
    | TTuple ts ->
  ...
  let types = Gamma.insert_exp erel trel g
              |> Gamma.types
              |> (List.dedup_and_sort ~compare:cheat_compare)
...
and gen_eexp_rel_app (tmo:Timeout.t) (s:Sigma.t)
  ...
  let rec extract_producer (t:typ) (u:typ) : typ option =
    match u with
    | TArr (_, t2) -> if equal_typ t2 t then Some u else extract_producer t t2
    | TBase _ -> None
    | TTuple _ -> None
    | TRcd _ -> None
    | TUnit -> None
