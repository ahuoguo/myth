...
let rec find_first_branch (c:id) (bs:branch list) : branch =
  ...
  | ((c', x), e)::bs -> if cheat_compare c c' = 0 then ((c', x), e) else find_first_branch c bs
...
let rec eval (env:env) (e:exp) : value =
  ...
  | None ->
      ...
      | EApp (e1, e2) ->
            ...
            | VPFun vps ->
                begin match Util.find_first (fun (v1, _) -> cheat_compare v1 v2 = 0) vps with
                | Some (_, v) -> v
                | None ->
