...
module Gamma_Struct (Dict : Dictionary) : Gamma_Sig = struct
    ...
    let rec cmp_exp e1 e2 = match (e1, e2) with
      | (EVar s1, EVar s2) -> cheat_compare s1 s2 = 0
      | (EProj (n1, e1'), EProj (n2, e2')) -> n1 = n2 && cmp_exp e1' e2'
      | (ERcdProj (l1, e1'), ERcdProj (l2, e2')) -> cheat_compare l1 l2 = 0 && cmp_exp e1' e2'
      | _ -> false

    let empty = {
        active     = Dict.empty cmp_exp;
        ids        = Dict.empty (fun x y -> cheat_compare x y = 0);
        recent     = [];
        fun_to_arg = Dict.empty (fun x y -> cheat_compare x y = 0);
        arg_to_fun = Dict.empty (fun x y -> cheat_compare x y = 0);
    }
    ...
    let mark_as_rec_fun_with_arg (f:id) (x:id) (g:t) =
      let s = match Dict.find f g.fun_to_arg with
              | Some s -> s
              | None -> Set.empty (fun x y -> cheat_compare x y = 0)
      in
    ...
    let mark_as_dec_arg_of_fun (x:id) (f:id) (g:t) =
      ...
      | Some (f', _) ->
        if String.(=) f f' then { g with arg_to_fun = Dict.set x (f, true) g.arg_to_fun }
        else failwith (Printf.sprintf "%s is arg of %s and can't be marked for %s" x f' f)
    ...
    let active_of_type (t:typ) (g:t) =
        let dict_of_type = Dict.filter (fun _ v -> equal_typ v t) g.active in
